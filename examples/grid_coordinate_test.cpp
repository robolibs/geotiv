...
int main() {
    ...
    concord::Pose shift{concord::Point{100.0, 200.0, 50.0}, concord::Euler{0.0, 0.0, 0.1}}; // Same shift as Layer demo
    ...
    // Create the 2D grid
    concord::Grid<uint8_t> originalGrid(rows, cols, cellSize, true, shift);
    ...
    std::cout << "Point (0,0): (" << origPoint00.x << ", " << origPoint00.y << ", " << origPoint00.z << ")" << std::endl;
    std::cout << "Point (0,1): (" << origPoint01.x << ", " << origPoint01.y << ", " << origPoint01.z << ")" << std::endl;
    std::cout << "Point (1,0): (" << origPoint10.x << ", " << origPoint10.y << ", " << origPoint10.z << ")" << std::endl;
    ...
    try {
        ...
        std::cout << "Cell size: " << reconstructedGrid.inradius() << "m" << std::endl;
        std::cout << "RC shift: (" << rc.shift.point.x << ", " << rc.shift.point.y << ", " << rc.shift.point.z << ")" << std::endl;
        ...
        std::cout << "Point (0,0): (" << reconPoint00.x << ", " << reconPoint00.y << ", " << reconPoint00.z << ")" << std::endl;
        std::cout << "Point (0,1): (" << reconPoint01.x << ", " << reconPoint01.y << ", " << reconPoint01.z << ")" << std::endl;
        std::cout << "Point (1,0): (" << reconPoint10.x << ", " << reconPoint10.y << ", " << reconPoint10.z << ")" << std::endl;
        
        // Compare coordinates
        std::cout << "\nCoordinate comparison:" << std::endl;
        
        auto checkPoint = [](const char* name, concord::Point orig, concord::Point recon) {
            bool matches = (std::abs(orig.x - recon.x) < 0.001 &&
                           std::abs(orig.y - recon.y) < 0.001 &&
            ...
            if (!matches) {
                ...
                std::cout << "  Difference: (" << (recon.x - orig.x) << ", " << (recon.y - orig.y) << ", " << (recon.z - orig.z) << ")" << std::endl;
            }
        };
        ...
        for (size_t r = 0; r < rows; ++r) {
            for (size_t c = 0; c < cols; ++c) {
                if (originalGrid(r, c) != reconstructedGrid(r, c)) {
                    dataMatches = false;
                    std::cout << "Data mismatch at (" << r << "," << c << "): " 
                             << (int)originalGrid(r, c) << " vs " << (int)reconstructedGrid(r, c) << std::endl;
                    break;
                }
            }
