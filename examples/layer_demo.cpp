...
int main() {
    ...
    dp::Pose shift{dp::Point{100.0, 200.0, 50.0}, rotation}; // 100m east, 200m north, 50m up, slight rotation
    ...
    // Create the 3D layer
    auto originalLayer = dp::make_layer<uint8_t>(rows, cols, layers, cellSize, layerHeight, true, shift, uint8_t{0});
    ...
    for (size_t l = 0; l < layers; ++l) {
        for (size_t r = 0; r < rows; ++r) {
            for (size_t c = 0; c < cols; ++c) {
                ...
                uint8_t value = static_cast<uint8_t>((l * 60) +             // Base value per layer
    ...
    for (size_t l = 0; l < layers; ++l) {
        ...
        std::cout << "Layer " << l << " corner (0,0): (" << point.x << ", " << point.y << ", " << point.z << ")"
                  << std::endl;
    }
    ...
    try {
        ...
        std::cout << "Dimensions: " << reconstructedLayer.rows << "x" << reconstructedLayer.cols << "x"
                  << reconstructedLayer.layers << std::endl;
        std::cout << "Cell size: " << reconstructedLayer.resolution << "m" << std::endl;
        ...
        for (size_t l = 0; l < layers; ++l) {
            ...
            std::cout << "Layer " << l << " corner (0,0): (" << reconPoint.x << ", " << reconPoint.y << ", "
                      << reconPoint.z << ")";

            // Check if coordinates match
            bool matches =
                (std::abs(origPoint.x - reconPoint.x) < 0.001 && std::abs(origPoint.y - reconPoint.y) < 0.001 &&
                 std::abs(origPoint.z - reconPoint.z) < 0.001);
            std::cout << (matches ? " OK" : " MISMATCH") << std::endl;
        }
                    ...
                    if (originalLayer(r, c, l) != reconstructedLayer(r, c, l)) {
                        ...
                        if (mismatchCount <= 3) { // Only show first few mismatches
                            std::cout << "Data mismatch at (" << r << "," << c << "," << l
                                      << "): " << (int)originalLayer(r, c, l) << " vs "
                                      << (int)reconstructedLayer(r, c, l) << std::endl;
                        }
                    }
                }
            }
